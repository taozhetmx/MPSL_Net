...
class F_Concat2(nn.Module):
    ...
    def forward(self, x):
        ...
        if w.numel() < C_total:
            # 不够 → 自动补齐
            pad = torch.ones(C_total - w.numel(), device=w.device, dtype=w.dtype)
            w_full = torch.cat([w, pad], dim=0)
        else:
            # 足够 → 自动裁剪
            w_full = w[:C_total]

        # --- 2. 归一化，确保 w1+w2 = 1 ---
        weight = w_full / (w_full.sum() + self.eps)

        # --- 3. 分成两段 ---
        w1 = weight[:C1].view(1, C1, 1, 1)
        w2 = weight[C1:].view(1, C2, 1, 1)

        # --- 4. 广播方式缩放，不用 view，不要求连续 ---
        x1_scaled = x1 * w1 + x1 * self.alpha
        x2_scaled = x2 * w2 + x2 * self.alpha

        # --- 5. 拼接 ---
        out = torch.cat([x1_scaled, x2_scaled], dim=self.d)
