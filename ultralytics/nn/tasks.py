...
from ultralytics.nn.extra_modules.conv import *
    ...
    base_modules = frozenset(
        {
            ...
            # 自定义模块
            FDConv_BN,DSConv,GSConv,C2f_xiaobo,DFM,SNI,TFF,FusionConv,gatedFusion,GSConvE,gatedFusion2,gatedFusion3,F2f,EMA,
            PSAplus,StripConv,FusionConv2,FC2f,GhostConv,ECA,C2fGhost,DWConv,GSConvE2,FusionConv3,GSConvE3,GSConvE4,GSConvE5,
            F_Concat,GConv,WDown,WD2
        }
    )
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            if isinstance(f, list):
                c1 = sum([ch[i] for i in f])  # 累加输入通道数
            else:
                c1 = ch[f]
            c2 = args[0]
            ...
            {Detect, SDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect}
        ):
            args.append([ch[x] for x in f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, SDetect, Segment, YOLOESegment, Pose, OBB}:
                m.legacy = legacy
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, SDetect, v10Detect)):
                return "detect"
